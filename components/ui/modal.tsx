    ...
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        ...
        <Dialog.Content asChild>
          ...
          >
            <div className="relative w-full max-w-lg rounded-xl bg-white border border-slate-200 shadow-lg p-6">
              {title && (
              ...
              {showCloseButton && (
                <Dialog.Close asChild>
                  <button
                    aria-label="Close"
                    className="absolute top-3 right-3 rounded-md p-1 text-slate-500 hover:bg-slate-100"
                  >
                    <X className="h-4 w-4" />
                  </button>
                </Dialog.Close>
